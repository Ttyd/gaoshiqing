...
//position link
const lj = (serverConfig.protocol || 'http')  + '://' + serverConfig.host  + ((serverConfig.port === '' || serverConfig.port === '80' || serverConfig.port === undefined) ? '' : ':' + serverConfig.port) + '/' + (serverConfig.path ? serverConfig.path : '');
...
gulp.task('image',function(){
    ...
    gulp.src('./app/**/*.{png,jpg,gif,ico}')
        ...
        .pipe(gulpif(options.env !== 'dev',gulp.dest('build')))
        .pipe(connect.reload());
});
...
gulp.task('css', function() {
    gulp.src('./app/**/*.css')
        .pipe(plumber())
        .pipe(gulpif(options.env !== 'dev',spriter({
    ...
    glob('./app/**/*.html', function (er, files) {
        ...
        for(var i = 0; i <files.length; i++){
            ...
            gulp.src(files[i])
                ...
                .pipe(gulpif(!bool && options.env !== 'dev', gulp.dest(paths[0] +'build' + newPath1)))
                .pipe(connect.reload());
        }
...
gulp.task('watch',function(){
    ...
    if(options.env === 'dev'){
        opn(lj+url, {app: ['chrome'],wait:true});
    }
});
